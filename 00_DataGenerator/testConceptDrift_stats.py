...
fn3 = os.path.join(data_fn, 'simulation_data_reduced.h5')
sim_data = pd.read_hdf(fn3, key='df')
...
stats_manipulation = [
    {
        'feature': 'line_7_8_loading',
        'at_time_idx': 96 * 31,
        'until_time_idx': 96 * 120,
        'shift_mean_by': 3.0,
        'shift_std_by': 4.0
    }
]

concept_drift = {'manipulate_switch': manipulate_switch, 'load_mapping': load_mapping,
                 'change_cos_phi': change_cos_phi, 'stats': stats_manipulation}
...
try:
    ...
    drifter.manipulate_by_stats(sim_data)
